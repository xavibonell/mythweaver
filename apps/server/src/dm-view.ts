 ...
 */
import { spatialIndex } from '@mythweaver/engine';
...
export function dmViewAnnotations(map: SceneMap, actingPcName?: string): { labels: LabelSpec[]; rings: RingSpec[] } {
  ...
  // Building-type plaques FIRST (actor plaques must win any overlap), anchored at the footprint's TOP
  // edge — a centroid plaque sits right where occupants/furniture stand and can HIDE a keeper from the
  // vision model (caught live: "is anyone inside the storehouse?" → "no", the plaque covered him).
  try {
...
export function renderDmView(map: SceneMap, assetsRoot: string, actingPcName?: string): string | undefined {
  try {
    ...
    return renderSceneMapToPng(map, { assetsRoot, showRoofs: false, neutralLighting: true, annotations }).toString('base64');
  } catch {
    return undefined;
  }
}
