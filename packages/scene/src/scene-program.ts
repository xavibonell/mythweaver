...
THEME (REQUIRED): pick ONE that fits the mood — village | forest | swamp | jungle | dungeon | crypt | necropolis | temple | cave | desert | lava | arctic | blight | infernal | fortress | sewer | underdark | mine. It sets ONE coherent floor/wall palette for the WHOLE scene (arctic = snowfield + ice walls, lava = ash + obsidian, necropolis = dark marble + bone walls, infernal = ash + hell-stone walls, fortress = fitted battlements, sewer = grimy acid-etched drains, underdark = the deep roads, mine = rough-hewn shafts…), so you do NOT pick ground/floor tags per op (the engine fills ground/path/plaza/room-floors from the theme). Only specify a tag for a HAZARD region (water / water_deep / lava / swamp) or an explicit biome FEATURE patch (ice / mud / snow / farmland / blight) — everything else is themed automatically.
...
export function normalizeProgram(raw: unknown, brief: string, moodText: string = brief, kindHint?: SceneKindHint): SceneProgram {
  ...
  // WILD ROUTING: an open-outdoor FOREST is composed by the deterministic forest generator (density-
  // gradient canopy + composed floor + clearings + a trail), NOT by LLM fill+random-scatter (which comes
  // out as trees sprinkled on a flat field — the biome-quality gap). Harvest the named cast, drop the
  ...
  const routedForest = !routedTown && !hasArchetype && !dominantWater && openWild && forestish;
  if (routedForest) {
    const contents = harvestTownContents(ops, lcb);
    ops.length = 0;
  ...
  if (!routedTown && !routedForest && !hasArchetype) {
    // STRUCTURE-COMPLETENESS NET: an interior/dungeon/cave/maze brief MUST have a structural backbone —
    // if the LLM emitted none (e.g. a "dungeon" as flat fill + scattered monsters), inject the right one
    // so it can never come out a flat field. Inserted BEFORE the first object op (so terrain fills stay
    // the base and objects land in the carved structure). Deterministic, no extra LLM call.
